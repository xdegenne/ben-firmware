"""
test_v05_store.py — Intégration v0x05 → measurements → /curve (db.py réel, sqlite seul).
...
le chemin de stockage de `on_recv_curve()` sans dépendance hardware.
    ...
    python3 src/pi/lora-receiver/test_v05_store.py
"""
...
import db                       # noqa: E402
from test_v05_roundtrip import KEY, encode_v05  # noqa: E402
...
def main():
    ...
    conn = db.connect(tmp)
    try:
        # Batch HCHP de 30 échantillons (PAPP variable, dont un gros saut), cadence 2 s.
        samples = [500 + (i * 37 % 400) for i in range(29)] + [2600]
        decoded = cc.decode_v04(
            encode_v05(samples, index_id=0x02, index_value=9_000_000, batch_seq=1), KEY)
